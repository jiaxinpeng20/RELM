...
#if RELM
double sigma = 2.0f;
//double prev_damping = 0.0f;
damping = std::pow(__initial_mse, sigma) * 0.01f; 
std::cout<<damping<<"  "<<__initial_mse<<std::endl;
#endif
    ...
    std::ofstream of;
    of.open("convergence.PBA4", std::ios::app); 
    of<<0<<"  "<<double(0.00f)<<"  "<<__initial_mse<<std::endl;
    for(int i = 0;    i < __lm_max_iteration  &&  !__abort_flag ; __current_iteration = (++i))
    {
        ...
        of<<i+1<<"\t"<<BundleTimerGetNow()<<"\t"<<double(_projection_sse * mse_convert_ratio)<<std::endl;
...
//double xi = 0.0f;
//if(1+xi*prev_damping <= 0) xi = 0;
double temp = (2.0f*gain_ratio-1.0f)*(2.0f*gain_ratio-1.0f)*(2.0f*gain_ratio-1.0f);
double q = 0.25f>1-2.0f*temp?0.25:1-2.0f*temp;
damping = 0.01f*q*std::pow(_projection_sse * mse_convert_ratio, sigma);damping_adjust = 2.0f;
